...
| `mentors/PROTOCOLS.md` | `core/PROTOCOLS.md` | Trellis adds INTAKE, "Keeping your notebook in git", CONFIG references, PROTOCOL_MODE, "Keeping Trellis in sync". PA's TRELLIS_SYNC protocol is not ported — this file replaces it. |
...
| `mentors/coordinator_state.md` — shape | `core/coordinator_state.md.template` | Sections incl. `## Active Watches — for the next review only`, `## Team board`, the fold; history to `mentors/coordinator_history/`. |
